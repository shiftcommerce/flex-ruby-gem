...
RSpec.describe FlexCommerce::OrderShippingMethod do
  ...
  let(:subject_class) { ::FlexCommerce::OrderShippingMethod }
  context "with data from factories" do
    context "working with multiple shipping methods" do
      let(:resource_list) { build(:order_shipping_method_list, quantity: 50) }
      let(:quantity) { 50 }
      let(:total_pages) { 2 }
      ...
      before :each do
        stub_request(:get, "#{api_root}/order_shipping_methods.json_api").with(headers: { "Accept" => "application/vnd.api+json" }).to_return body: resource_list.to_h.to_json, status: response_status, headers: default_headers
      end
